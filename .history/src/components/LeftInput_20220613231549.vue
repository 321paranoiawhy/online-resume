...
export default {
    ...
    setup() {
        ...
        let compileInput = () => {
            ...
            if (projectText) {
                // 清空后再逐个添加
                // console.log(document.querySelectorAll("h4.project"));
                // document.querySelector("ul.award").innerHTML = "";

                document
                    .querySelectorAll("h4.project")
                    .forEach(
                        (item, index) => (item.innerHTML = projectText[index])
                    );
                // for (let i = 0; i < projectText.length; i++) {
                //     let newElement = document.createElement("h4");
                //     newElement.className = "project";
                //     newElement.innerHTML = projectText[i].slice(
                //         "*project: ".length
                //     );
                //     document
                //         .querySelector("h4.project")
                //         .appendChild(newElement);
                // }
            }
