...
export default {
    ...
    setup() {
        ...
        let compileInput = () => {
            ...
            //     document
            //         .querySelectorAll("li.website")
            //         .forEach((item, index) =>
            //             projectWebsiteText[index]
            //                 ? (item.innerHTML = projectWebsiteText[index].slice(
            //                       "*website: ".length
            //                   ))
            //                 : (item.innerHTML = null)
            //         );
            // }
            let projectFeatureText = splitArray.filter((item) =>
                item.startsWith("*feature: ")
            );
            if (projectFeatureText) {
                document
                    .querySelectorAll("li.feature")
                    .forEach((item, index) =>
                        projectFeatureText[index]
                            ? (item.innerHTML = projectFeatureText[index].slice(
                                  "*feature: ".length
                              ))
                            : (item.innerHTML = null)
                    );
