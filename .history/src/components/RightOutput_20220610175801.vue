        ...
        let Fn = (parameter) => {
            switch (parameter) {
                case "download":
                    ...
                    // 方法二: html2canvas + jspdf (html -> canvas -> dataURL -> pdf)
                    // 1. html2canvas (html -> canvas) 2. canvas.toDataURL() (canvas -> 图片) 3. jspdf (图片 -> pdf)
                    // 弊端: pdf实质上为图片, 无法选中文本, 只能选中图片, 且图片清晰度有所损失, 最终打印效果不佳
                    // 弊端: html2canvas 支持的 CSS 属性和值有限:
                    ...
                    // html2pdf: https://github.com/eKoopmans/html2pdf.js#usage
