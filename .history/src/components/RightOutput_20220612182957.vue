...
export default {
    ...
    setup() {
        window.onscroll = function () {
            const top = document.querySelector(".grid").offsetTop;
            const left = document.querySelector(".grid").offsetLeft;
            if (window.scrollY > document.querySelector(".grid").offsetTop) {
                // console.log("scrollY: " + window.scrollY);
                // console.log(output.value.offsetTop);
                document.querySelector(".grid").classList.add("fixed");
                document.querySelector(".grid.fixed").style.position = "fixed";
                document.querySelector(".grid.fixed").style.top = top + "px";
                document.querySelector(".grid.fixed").style.left = left + "px";
            } else {
                document.querySelector(".grid").classList.remove("fixed");
            }
        };
        ...
        let Fn = (parameter) => {
            switch (parameter) {
                case "download":
                    ...
                    // }
                    // // 将节点转换为 svg 图片
                    // // issue li::marker color 样式会被忽略掉, 除此之外, 近乎完美
                    domtoimage
                        .toSvg(node, {
                            filter: filter,
                            bgcolor: "white",
