...
export default {
    ...
    setup() {
        ...
        let compileInput = () => {
            ...
            let selfEvaluationCount =
                input.value.value.match(/\*self-evaluation:\s/gi).length;
            console.log(selfEvaluationCount);
            console.log(input.value.value.match(/\*self-evaluation:\s/gi));
            for (let i = 0; i < selfEvaluationCount; i++) {
                console.log(
                    input.value.value.match(/\*self-evaluation:\s/gi)[i]
                );
            }
            document.querySelector("ul.selfEvaluation").children[0].innerHTML =
                selfEvaluationCount;
